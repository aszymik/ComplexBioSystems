...
```{r homework 1}
pdf("szymik_problem1.pdf", height=10, width=10)
heatmap.2(edata,
...
```{r homework 2}
pdf("szymik_problem2.pdf", height=8, width=10)
ggplot(PC) + 
  geom_point(aes(x=V2, y=V3, col=as.factor(strain))) + 
  labs(
    title="Bottomly et al. PCA projection",
    x="PC 1",
    y="PC 2",
    color="Strain"
  )
...
pdf("szymik_problem3.pdf", height=8, width=10)
ggplot(loadings) + 
  geom_point(aes(x=V1, y=V2)) + 
  labs(
    title="Left singular vectors (loadings) visualisation",
    x="Loading 1",
    y="Loading 2"
  )
dev.off()
...
```{r homework 4}
loadings_5 = loadings[, .(V1, V2, V3, V4, V5)]

# Reshape data for plotting
loadings_long = melt(loadings_5, measure.vars = patterns("V"), variable.name = "PC", value.name = "loading")

pdf("szymik_problem4.pdf", height=8, width=10)
ggplot(loadings_long, aes(x = PC, y = loading)) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) +
  labs(title = "Violin plots of top 5 left singular vectors (loadings)",
       x="Component",
       y="Loading value")
dev.off()
```
...
```{r homework 5}
set.seed(17)
kmeans_results <- kmeans(edata, centers=5, nstart=20)
clusters <- as.factor(kmeans_results$cluster)

# Add cluster labels
tsne_out[, Cluster := clusters]

pdf("szymik_problem5.pdf", height=8, width=10)
ggplot(tsne_out, aes(x=V1, y=V2, color=Cluster)) +
  geom_point(alpha = 0.7) +
  labs(
    title = "t-SNE projection of Bottomly et al. data colored by K-means clusters",
    x = "t-SNE 1",
    y = "t-SNE 2",
