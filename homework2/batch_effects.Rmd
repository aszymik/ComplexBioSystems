...
# Group by unique values for each batch
batch_outcomes <- pheno %>%
  group_by(batch) %>%
  summarise(outcomes = paste(unique(outcome), collapse = ", ")) %>%
  pivot_wider(names_from = batch, values_from = outcomes, names_prefix = "Batch ")

batch_cancer <- pheno %>%
  group_by(batch) %>%
  summarise(cancers = paste(unique(cancer), collapse = ", ")) %>%
  pivot_wider(names_from = batch, values_from = cancers, names_prefix = "Batch ")

# Combine dataframes and add row names
batch_summary <- data.frame(batch_outcomes, check.names = FALSE)
batch_summary <- rbind(batch_summary, batch_cancer)
row.names(batch_summary) <- c('Outcome', 'Cancer')

pdf('szymik_problem1.pdf', width = 10, height = 4)
...
# Sort the columns by batch
pheno_ordered <- pheno[order(pheno$batch), ]
edata_sorted <- edata[, pheno_ordered$sample]
combat_edata_sorted <- combat_edata[, pheno_ordered$sample]

pdf('szymik_problem2a.pdf', height=10, width=10)
heatmap.2(edata_sorted,
          ...
          scale = "row",
          ColSideColors = brewer.pal(5, "Set1")[as.factor(pheno_ordered$batch)])  # color columns by batch
dev.off()

pdf('szymik_problem2b.pdf', height=10, width=10)
...
```{r homework 3}
cor_matrix <- cor(edata, method = 'pearson', use = 'pairwise.complete.obs')
cor_labels <- paste(pheno$cancer, ', Batch', pheno$batch)

pdf('szymik_problem3.pdf', height=10, width=10)
...
```{r homework 4}
edata_bottomly <- as.matrix(exprs(bottomly.eset))
edata_bottomly <- edata_bottomly[rowMeans(edata_bottomly) > 10, ]
edata_bottomly <- log2(as.matrix(edata_bottomly) + 1)
pheno_bottomly <- pData(bottomly.eset)

# Linear model with uncorrected gene expression data
mod_uncorr <- lm(t(edata_bottomly) ~ as.factor(pheno_bottomly$strain) + as.factor(pheno_bottomly$experiment.number))
mod_uncorr_tidy <- tidy(mod_uncorr)

combat_bottomly <- ComBat(dat=edata_bottomly, batch=pheno_bottomly$experiment.number, mod=model.matrix(~1, data=pheno_bottomly), par.prior=TRUE)
mod_combat_bottomly <- lm(t(combat_bottomly) ~ as.factor(pheno_bottomly$strain))
mod_combat_tidy <- tidy(mod_combat_bottomly)

# Compare coefficients
est_compare <- tibble(
  LinearModel = mod_uncorr_tidy %>% filter(term == "as.factor(pheno_bottomly$strain)DBA/2J") %>% select("estimate") %>% unlist,
  ComBat = mod_combat_tidy %>% filter(term == "as.factor(pheno_bottomly$strain)DBA/2J") %>% select("estimate") %>% unlist)

pdf('szymik_problem4a.pdf', height=10, width=10)
ggplot(est_compare, aes(x=LinearModel, y=ComBat)) + 
...
# P-values for the uncorrected model
pdf('szymik_problem4b.pdf', height=10, width=10)
ggplot(mod_uncorr_tidy %>% filter(term == "as.factor(pheno_bottomly$strain)DBA/2J")) + 
    geom_histogram(aes(x=p.value), bins = 100, fill="darkorange")
dev.off()

# P-values for the combat--corrected model
pdf('szymik_problem4c.pdf', height=10, width=10)
ggplot(mod_combat_tidy %>% filter(term == "as.factor(pheno_bottomly$strain)DBA/2J")) + 
    geom_histogram(aes(x=p.value), bins = 100, fill="darkorange")
dev.off()
```
...
```{r homework 5}
# Combat p-values
pdf('szymik_problem5a.pdf', height=10, width=10)
ggplot(mod_combat_tidy %>% filter(term == "as.factor(pheno_bottomly$strain)DBA/2J")) + geom_histogram(aes(x=p.value), bins = 100, fill="darkorange") + labs(title='Distribution of ComBat p-values')
dev.off()

# SVA
mod_sva_bottomly = model.matrix(~as.factor(strain),data=pheno_bottomly)
mod_sva_0 = model.matrix(~1, data=pheno_bottomly)
sva_output = sva(edata_bottomly, mod_sva_bottomly, mod_sva_0, n.sv=num.sv(edata_bottomly, mod_sva_bottomly, method="leek"))

# P-values histogram
mod_sva = lm(t(edata_bottomly) ~ as.factor(pheno_bottomly$strain) + sva_output$sv)
mod_sva_tidy <- tidy(mod_sva)

pdf('szymik_problem5b.pdf', height=10, width=10)
ggplot(mod_sva_tidy %>% filter(term == "as.factor(pheno_bottomly$strain)DBA/2J")) + geom_histogram(aes(x=p.value), bins = 100, fill="darkorange") + labs(title='Distribution of SVA p-values')
dev.off()

# Scatter plot of coefficients
est_compare <- tibble(
  SVA = mod_sva_tidy %>% filter(term == "as.factor(pheno_bottomly$strain)DBA/2J") %>% select("estimate") %>% unlist,
  ComBat = mod_combat_tidy %>% filter(term == "as.factor(pheno_bottomly$strain)DBA/2J") %>% select("estimate") %>% unlist)

pdf('szymik_problem5c.pdf', height=10, width=10)
