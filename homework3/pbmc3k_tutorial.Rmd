...
```{r load data}
# Load the dataset
pbmcZheng.data <- Read10X(data.dir = "filtered_matrices_zheng/hg19/")

# Initialize the Seurat object with the raw (non-normalized data)
pbmcZheng <- CreateSeuratObject(counts = pbmcZheng.data, project = "pbmc68k", min.cells = 3, min.features = 200)
pbmcZheng

rm(pbmcZheng.data) # clean memory
gc()
```
...
```{r qc, fig.height=7, fig.width=13}
# Stash QC stats
pbmcZheng[["percent.mt"]] <- PercentageFeatureSet(pbmcZheng, pattern = "^MT-")

# Visualize QC metrics as a violin plot
VlnPlot(pbmcZheng, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol=3, alpha=0.1)

FeatureScatter(pbmcZheng, feature1 = "nCount_RNA", feature2 = "percent.mt") + FeatureScatter(pbmcZheng, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
```

Based on the plots, let's filter the cells that have unique feature counts over 1,500 or less than 200 and the ones that have more than 5% mitochondrial counts.

```{r cell_selection}
pbmcZheng <- subset(pbmcZheng, subset = nFeature_RNA > 200 & nFeature_RNA < 1500 & percent.mt < 5)
```

## Normalizing the data

```{r normalize}
pbmcZheng <- NormalizeData(pbmcZheng)
```

## Highly variable features selection

```{r var_features, fig.height=5, fig.width=11}
pbmcZheng <- FindVariableFeatures(pbmcZheng, selection.method = 'vst', nfeatures = 1000)  # top 1,000 variable genes as Zheng et al. have done
```

## Scaling the data

```{r regress, fig.height=7, fig.width=11, results='hide'}
pbmcZheng <- ScaleData(pbmcZheng)  # using all genes caused R termination
```

## PCA

```{r pca, results='hide'}
pbmcZheng <- RunPCA(pbmcZheng) # PCA on top 1,000 variable features
```

```{r pca_viz, message=TRUE}
# Examine and visualize PCA results a few different ways
print(pbmcZheng[['pca']], dims = 1:5, nfeatures = 5)
VizDimLoadings(pbmcZheng, dims = 1:2, reduction = 'pca')
DimPlot(pbmcZheng, reduction = 'pca')
DimHeatmap(pbmcZheng, dims = 1:15, cells = 500, balanced = TRUE)
```
...
```{r jackstraw, fig.height=6, fig.width=10}
# NOTE: This process can take a long time for big datasets, comment out for expediency. More approximate techniques such as those implemented in ElbowPlot() can be used to reduce computation time
pbmcZheng <- JackStraw(pbmcZheng, num.replicate = 50)
pbmcZheng <- ScoreJackStraw(pbmcZheng, dims = 1:20)  # thresholding
```

```{r jsplots, fig.height=6, fig.width=10}
JackStrawPlot(pbmcZheng, dims = 1:20)
```
...
Based on the above plots, I would choose about 12 PCs as a cutoff. However, in their analysis Zheng et al. used 50 PCs, so let's stick to this number to try to recreate their results.

## K-means clustering

```{r kmeans}
pca.embeddings <- Embeddings(pbmcZheng, reduction = "pca")[, 1:50]

# K-means clustering with 10 clusters
set.seed(412)
kmeans.result <- kmeans(pca.embeddings, centers = 10)

# Add K-means cluster labels to the Seurat object
pbmcZheng$kmeans_clusters <- as.factor(kmeans.result$cluster)

# Set the K-means clusters as active identity
Idents(pbmcZheng) <- "kmeans_clusters"

# Look at cluster IDs of the first 5 cells
head(Idents(pbmcZheng), 5)
```

## Run tSNE

```{r tsne, fig.height=5, fig.width=7}
pbmcZheng <- RunTSNE(pbmcZheng, dims = 1:12)  # running tSNE on first 12 PCs gave much better results than 50 like k-means
```
...
# First, calculate cluster sizes and percentages
cluster_freq <- table(pbmcZheng$kmeans_clusters)
cluster_percent <- round(100 * cluster_freq / sum(cluster_freq), 1)

# Create cluster labels with percentages
cluster_labels <- paste0(names(cluster_freq), " (", cluster_percent, "%)")
names(cluster_labels) <- names(cluster_freq)

# Replace cluster IDs with formatted labels in Seurat object
pbmcZheng$kmeans_clusters_labeled <- plyr::mapvalues(pbmcZheng$kmeans_clusters,
                                                     from = names(cluster_labels),
                                                     to = cluster_labels)

# Set the new labeled clusters as identities
Idents(pbmcZheng) <- "kmeans_clusters_labeled"

png("figures/tsne.png", height=500, width=600)

# Plot tSNE with cluster labels
DimPlot(pbmcZheng, reduction = "tsne", label = TRUE, label.size = 4.5, pt.size = 0.5) +
  xlab("tSNE 1") + ylab("tSNE 2") +
  ggtitle("T-SNE with K-means clusters") +
  NoLegend()
dev.off()
...
```{r saveobject}
saveRDS(pbmcZheng, file = "pbmc_Zheng.rds")
```
...
subcluster_labels <- rep(NA, ncol(pbmcZheng))  # to keep all subcluster labels
names(subcluster_labels) <- colnames(pbmcZheng)

# Clustering for each main cluster
set.seed(42)
for (main_cluster in levels(pbmcZheng$kmeans_clusters)) {
  
  # Subset cells for current main cluster
  sub_cells <- subset(pbmcZheng, subset = kmeans_clusters == main_cluster)
  
  # Get PCA embeddings
  pca.sub <- Embeddings(sub_cells, reduction = "pca")[, 1:12]
  
  # Find best k using silhouette-like method
  sil_scores <- numeric()
  for (k in 2:6) {
    pam_fit <- pam(pca.sub, k = k)
    sil_scores[k] <- pam_fit$silinfo$avg.width
  }
  best_k <- which.max(sil_scores)
  message("Best k for cluster ", main_cluster, " is: ", best_k)
  
  # Final K-means with best k
  km.final <- kmeans(pca.sub, centers = best_k, nstart = 20)
  final_ids <- paste0(main_cluster, "_", km.final$cluster)
  
  # Store subcluster labels in original cell order
  subcluster_labels[colnames(sub_cells)] <- final_ids
}

# Add subcluster labels to the Seurat object
pbmcZheng$subcluster <- subcluster_labels
```

```{r visualize subclustering}
Idents(pbmcZheng) <- "subcluster"

main_clusters <- unique(pbmcZheng$kmeans_clusters)

for (main_cluster in main_clusters) {
  sub_cells <- subset(pbmcZheng, subset = kmeans_clusters == main_cluster)
  
  png(paste0("figures/tsne_cluster", main_cluster, ".png"), height=500, width=600)
  
  # tSNE colored by subcluster
  p <- DimPlot(sub_cells,
               reduction = "tsne",
               group.by = "subcluster",
               label = TRUE,
               repel = TRUE,
               pt.size = 0.6) +
       ggtitle(paste("Cluster", main_cluster, " subclusters"))
  print(p)
  dev.off()
}
...
```{r}
png("figures/tsne_all_subclusters.png", height=500, width=600)
DimPlot(pbmcZheng,
        reduction = "tsne",
        group.by = "subcluster",
        label = TRUE,
        repel = TRUE,
        pt.size = 0.5) +
  ggtitle("Overall t-SNE with subcluster labels") +
  NoLegend()
dev.off()
```
